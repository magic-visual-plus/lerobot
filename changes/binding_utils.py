...
class MjRenderContext:
    ...
    def read_pixels(self, width, height, depth=False, segmentation=False):
        ...
        if segmentation:
            ...
            seg_img = (rgb_img[:, :, 0].astype(np.uint32) +
                rgb_img[:, :, 1].astype(np.uint32) * 256 +
                rgb_img[:, :, 2].astype(np.uint32) * 65536)
            ...
            for i in range(self.scn.ngeom):
                ...
                if geom.segid != -1:
                    # print(f'geom.segid: {geom.segid}, geom.objtype: {geom.objtype}, geom.objid: {geom.objid}')
