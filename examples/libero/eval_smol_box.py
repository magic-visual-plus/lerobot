...
# from lerobot.policies.smolvla.modeling_smolvla import SmolVLAPolicy as SmolVLAPolicy
from lerobot.policies.smolvla3.modeling_smolvla3 import SmolVLA3Policy as SmolVLAPolicy
        ...
        for episode_idx in tqdm(
            ...
            while t < max_steps:
                try:
                    ...
                    # pad to 32
                    # observation['observation.state'] = torch.nn.functional.pad(
                    #     observation['observation.state'], (0, 32 - observation['observation.state'].shape[-1])
                    # )
                    # Query model to get action
                    start = time.time()
                    with torch.inference_mode():
                        res = policy.select_action(observation)
                        action_tensor = res["action"]
                        box = res["box"]
                        pass
                    # draw box
                    box0 = box[0, 0, :].cpu().numpy()
                    ...
                    frames.append(agentview_image)
                    # print(f"Inference time: {time.time() - start:.3f} seconds")
