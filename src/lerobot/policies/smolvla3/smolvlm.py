...
class SmolVLM(nn.Module):
    ...
    def forward_cross(
        self,
        pad_mask_key: torch.Tensor | None = None,
        position_ids_key: torch.LongTensor | None = None,
        input_embeds_key: torch.FloatTensor | None = None,
        pad_mask_query: torch.Tensor | None = None,
        position_ids_query: torch.LongTensor | None = None,
        input_embeds_query: torch.FloatTensor | None = None,
    ):
        attention_mask = pad_mask_key[:, None, :] * pad_mask_key[:, :, None]
        past_key_values = self.prepare_for_generation(
            attention_mask=attention_mask,
            position_ids=position_ids_key,
            inputs_embeds=input_embeds_key,
        )

        attention_mask_key = pad_mask_key[:, None, :] * pad_mask_query[:, :, None]
        attention_mask_query = pad_mask_query[:, None, :] * pad_mask_query[:, :, None]
                                                                           
        return self.forward(
            attention_mask_cross=attention_mask_key,
            attention_mask_self=attention_mask_query,
            position_ids=position_ids_query,
            inputs_embeds=input_embeds_query,
