...
class ObjectBoxDecoder(nn.Module):
    def __init__(self, hidden_dim, num_queries, num_classes):
        ...
        self.box_queries = nn.Parameter(torch.randn(1, num_queries, hidden_dim))
    ...
    def forward_loss(self, vlm, past_key_values, batch_size, box_target, class_target, mask_target):

        box_embeddings = vlm.forward(
            inputs_embeds=self.box_queries.expand(batch_size, -1, -1).to(vlm.dtype),
    ...
    def predict(self, vlm, past_key_values, batch_size, score_thresh=0.5):
        box_embeddings = vlm.forward(
            inputs_embeds=self.box_queries.expand(batch_size, -1, -1).to(vlm.dtype),
            past_key_values=DynamicCache.from_legacy_cache(past_key_values),
        ).last_hidden_state.to(torch.float32)   
