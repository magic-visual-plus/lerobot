...
from lerobot.policies.smolvla2.smolvlm import SmolVLM
...
class VLAFlowMatching(nn.Module):
    ...
    def __init__(self, config: SmolVLA2Config):
        super().__init__()
        self.config = config
    ...
    def forward(
        ...
        attention_masks = [prefix_pad_masks, suffix_pad_masks]
        position_ids = [prefix_position_ids, suffix_position_ids]
        (_, suffix_out), _ = self.vlm.forward(
    ...
    def denoise_step(
        ...
        outputs_embeds, _ = self.vlm.forward(
            attention_mask=[prefix_pad_masks, suffix_pad_masks],
            position_ids=[prefix_position_ids, suffix_position_ids],
            past_key_values=None,
            inputs_embeds=[prefix_embs, suffix_embs],
            use_cache=False,
